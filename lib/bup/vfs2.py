...
that is either an instance of Metadata or an integer mode, perhaps by
...
def augment_item_meta(repo, item, include_size=False):
    ...
    instance, this call may modify it in place, or replace it, so
    assume that any existing references to item.meta may be stale
    after this call.
