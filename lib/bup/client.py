...
class Client:
    ...
    def cat_batch(self, refs):
        ...
        for ref in refs:
            ...
            if info == 'missing\n':
                ...
                continue
            if (not info) or (not info.endswith('\n')):
                raise ClientError('Hit EOF while looking for object info')
    ...
    def refs(self, patterns=None, limit_to_heads=False, limit_to_tags=False):
        ...
        self._busy = 'refs'
        conn = self.conn
        conn.write('refs %s %s\n' % (bool(limit_to_heads), bool(limit_to_tags)))
        ...
        conn.write('\n')
        while True:
            name = read_lenprefix_line(conn, ClientError)
            oid = conn.read(20)
            assert conn.read(1) == '\n'
            yield name, oid
        # FIXME: confusing
